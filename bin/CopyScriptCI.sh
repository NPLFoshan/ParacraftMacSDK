...
dest_path=/Volumes/CODE/NPLRuntimeCI/NPLRuntime/Platform/iOS/assets # change to your project folder
paracraft_build_in_mod_path=/Volumes/CODE/ParacraftBuildinMod

pushd /Volumes/CODE/trunkCI
git pull origin dev --rebase
zip -r $dev_path/mainCI.zip ./ -x "./.git/*"
popd
...
cat >./create_pkg_ci.lua <<EOL
NPL.load("(gl)script/ide/commonlib.lua"); 

local filesPath = "${files_path}"
local destPath = "${dest_path}"
local originFile = "${dev_path}/mainCI.zip"
local encryptFile = "${dev_path}/mainCI.pkg"
...
ParaIO.DeleteFile(filesPath .. "/create_pkg_ci.lua")
...
cat >./config.txt <<EOL
cmdline=noupdate="true" bootstrapper="create_pkg_ci.lua" 
EOL
...
cp $dev_path/mainCI.pkg $dest_path/main.pkg
rm $dev_path/mainCI.pkg
